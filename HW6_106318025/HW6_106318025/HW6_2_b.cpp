...
void gaussian_lowpass_filter(Mat &dft_Filter, int D)
{
	...
	for (int i = 0; i < dft_Filter.rows; i++)
	{
		...
		for (int j = 0; j < dft_Filter.cols; j++)
		{
			...
			data[j] = (float)exp(-pow(radius, 2) / (2 * pow(D, 2)));
		}
	}
...
void gaussian_highpass_filter(Mat &dft_Filter, int D)
{
	Mat tmp = Mat(dft_Filter.rows, dft_Filter.cols, CV_32F);
	...
	for (int i = 0; i < dft_Filter.rows; i++)
	{
		...
		{
			radius = (double)sqrt(pow((i - centre.x), 2.0) + pow((double)(j - centre.y), 2.0));
			data[j] = 1 - (float)exp(-pow(radius, 2) /( 2 * pow(D, 2)));
		}
	}
...
void frequency_filtering(Mat &in, int radius, bool HPF_LPF, Mat &in_filter_magnitude, Mat &convert_in_filter_image, Mat &convert_filter_Output)//(HPF)bool HPF_LPF = true,(LPF)bool HPF_LPF = false
{
	...
	if (HPF_LPF == true)
	{
		gaussian_highpass_filter(filter, radius);//HPF is DC component
	}
	if (HPF_LPF == false)
	{
		gaussian_lowpass_filter(filter, radius);//LPF is DC component
	}
		...
		for (int i = 0; i < 3; i++)
		{
			if (flag == 0)
			{
				...
				case 2:
				{
					...
					fileout = fopen("HW6_2_b_LPF_D0=15_magnitude.raw", "wb");
					fwrite(Cat512_LPF_magnitude.data, Cat512_LPF_magnitude.rows*Cat512_LPF_magnitude.cols, 1, fileout);
					fclose(fileout);
					fileout = fopen("HW6_2_b_LPF_D0=15_image.raw", "wb");
					fwrite(Cat512_LPF_image.data, Cat512_LPF_image.rows*Cat512_LPF_image.cols, 1, fileout);
					fclose(fileout);
					fileout = fopen("HW6_2_b_LPF_D0=15_filter.raw", "wb");
					fwrite(LPF_Output.data, LPF_Output.rows*LPF_Output.cols, 1, fileout);
					fclose(fileout);
				}
				case 3:
				{
					...
					fileout = fopen("HW6_2_b_LPF_D0=50_magnitude.raw", "wb");
					fwrite(Cat512_LPF_magnitude.data, Cat512_LPF_magnitude.rows*Cat512_LPF_magnitude.cols, 1, fileout);
					fclose(fileout);
					fileout = fopen("HW6_2_b_LPF_D0=50_image.raw", "wb");
					fwrite(Cat512_LPF_image.data, Cat512_LPF_image.rows*Cat512_LPF_image.cols, 1, fileout);
					fclose(fileout);
					fileout = fopen("HW6_2_b_LPF_D0=50_filter.raw", "wb");
					fwrite(LPF_Output.data, LPF_Output.rows*LPF_Output.cols, 1, fileout);
					fclose(fileout);
				}
				default:
			...
			if (flag == 1)
			{
				...
				case 2:
				{
					...
					fileout = fopen("HW6_2_b_HPF_D0=15_magnitude.raw", "wb");
					fwrite(Cat512_HPF_magnitude.data, Cat512_HPF_magnitude.rows*Cat512_HPF_magnitude.cols, 1, fileout);
					fclose(fileout);
					fileout = fopen("HW6_2_b_HPF_D0=15_image.raw", "wb");
					fwrite(Cat512_HPF_image.data, Cat512_HPF_image.rows*Cat512_HPF_image.cols, 1, fileout);
					fclose(fileout);
					fileout = fopen("HW6_2_b_HPF_D0=15_filter.raw", "wb");
					fwrite(HPF_Output.data, HPF_Output.rows*HPF_Output.cols, 1, fileout);
					fclose(fileout);
				}
				case 3:
				{
					...
					fileout = fopen("HW6_2_b_HPF_D0=50_magnitude.raw", "wb");
					fwrite(Cat512_HPF_magnitude.data, Cat512_HPF_magnitude.rows*Cat512_HPF_magnitude.cols, 1, fileout);
					fclose(fileout);
					fileout = fopen("HW6_2_b_HPF_D0=50_image.raw", "wb");
					fwrite(Cat512_HPF_image.data, Cat512_HPF_image.rows*Cat512_HPF_image.cols, 1, fileout);
					fclose(fileout);
					fileout = fopen("HW6_2_b_HPF_D0=50_filter.raw", "wb");
					fwrite(HPF_Output.data, HPF_Output.rows*HPF_Output.cols, 1, fileout);
					fclose(fileout);
				}
				default:
